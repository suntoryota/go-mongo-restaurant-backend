...
import (
	...
	"github.com/suntoryota/go-restaurant/database"
	"github.com/suntoryota/go-restaurant/models"
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	groupStage := bson.D{{Key: "$group", Value: bson.D{{Key: "_id", Value: bson.D{{Key: "order_id", Value: "$order_id"}, {Key: "table_id", Value: "$table_id"}, {Key: "table_number", Value: "$table_number"}}}, {Key: "payment_due", Value: bson.D{{Key: "$sum", Value: "$amount"}}}, {Key: "total_count", Value: bson.D{{Key: "$sum", Value: 1}}}, {Key: "order_iteValue: ms", Value: bson.D{{Key: "$push", Value: "$$ROOT"}}}}}}
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := orderItemCollection.FindOne(ctx, bson.M{"order_item_id": orderItemsId}).Decode(&orderItem)
		defer cancel()
		if err != nil {
